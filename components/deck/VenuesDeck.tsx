...
// Insert a width constraint into a Cloudinary URL for smaller thumbnails.
...
const NAV_LABELS = ["Overview", "The Arena", "The Playhouse", "The Expo", "The Pavilion"];
...
function CinematicBg({ src }: { src: string }) {
  return (
    <>
      ...
      />
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(10,15,30,0.70) 0%, rgba(10,15,30,0.22) 42%, rgba(10,15,30,0.80) 100%)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(105deg, rgba(10,15,30,0.82) 0%, rgba(10,15,30,0.35) 45%, transparent 70%)", pointerEvents: "none" }} />
...
function SpecCard({ value, label, foot, delay = 0 }: { value: string; label: string; foot?: string; delay?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: "clamp(32px, 4vw, 80px)", marginBottom: "clamp(28px, 4vh, 48px)", flexWrap: "wrap" }}>
          ...
          >
            Arena, theatre, exhibition floor, private entertaining — every
            scale, every format, every audience. All inside the destination.
          </motion.p>
        </div>
          ...
          {VENUE_ORDER.map((slug, i) => {
            ...
            return (
              ...
              >
                ...
                />
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(140deg, rgba(10,15,30,0.82) 0%, rgba(10,15,30,0.30) 45%, rgba(10,15,30,0.55) 100%)", pointerEvents: "none" }} />
...
function VenueSlide({
  ...
  const v = VENUES[slug];
  const topPast = v.past.slice(0, 3);
  ...
  return (
    ...
    >
      ...
      >
        {/* LEFT: text + past events */}
        <div style={{ flex: "0 1 580px", minWidth: 0, maxWidth: "620px" }}>
          <Eyebrow text={`${v.index} — ${v.eyebrow}`} />
          <Headline>
            {v.hero.headline}
            {v.hero.headlineAccent && (
              <>
                <br />
                <span style={{ color: "#C9A96E" }}>{v.hero.headlineAccent}</span>
              </>
            )}
          </Headline>
          <GoldRule />
          <Body>{v.hero.subhead}</Body>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              {topPast.map((p, i) => (
                <motion.div
                  ...
                  transition={{ duration: 0.4, ease: EASE, delay: 0.38 + i * 0.06 }}
                  style={{
                    display: "grid",
                ...
                >
                  <span style={{ fontFamily: "var(--font-montserrat)", fontSize: "10px", fontWeight: 600, letterSpacing: "0.12em", color: "rgba(201,169,110,0.75)" }}>
                    {p.year}
                  </span>
                  <span style={{ fontFamily: "var(--font-montserrat)", fontSize: "13px", fontWeight: 500, color: "rgba(255,255,255,0.78)", letterSpacing: "-0.005em" }}>
                    {p.title}
                  </span>
                  <span style={{ fontFamily: "var(--font-montserrat)", fontSize: "10px", color: "rgba(255,255,255,0.35)", letterSpacing: "0.03em", textAlign: "right" }}>
                    {p.note}
                  </span>
                </motion.div>
              ))}
        ...
        >
          ...
          {/* First production inclusion as highlight card */}
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            style={{
              ...
              border: "1px solid rgba(255,255,255,0.08)",
              borderRadius: "16px",
              padding: "clamp(18px, 2.5vh, 24px) clamp(20px, 2vw, 26px)",
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
              <div style={{ width: "18px", height: "1px", background: "rgba(201,169,110,0.60)" }} />
              <span style={{ fontFamily: "var(--font-montserrat)", fontSize: "9px", fontWeight: 600, letterSpacing: "0.30em", textTransform: "uppercase", color: "rgba(201,169,110,0.75)" }}>
                Production Included
              </span>
            </div>
            <div style={{ fontFamily: "var(--font-montserrat)", fontSize: "13px", fontWeight: 600, color: "rgba(255,255,255,0.82)", marginBottom: "8px", letterSpacing: "-0.005em" }}>
              {v.production[0].title}
            </div>
            <div style={{ fontFamily: "var(--font-montserrat)", fontSize: "11.5px", lineHeight: 1.6, color: "rgba(255,255,255,0.50)", letterSpacing: "0.005em" }}>
              {v.production[0].body}
            </div>
          </motion.div>
        </div>
...
export default function VenuesDeck() {
  ...
  useEffect(() => {
    if (presentation.state.isPresenting && presentation.state.currentDeck === "venues") {
  ...
  const openForVenue = useCallback(
    (slug: VenueSlug) => {
      open(VENUES[slug].inquiryType);
    },
    [open],
  );
  ...
  const slides = [
    ...
    <VenueSlide key="arena" slug="arena" index={1} openInquiry={() => openForVenue("arena")} />,
    <VenueSlide key="performing-arts" slug="performing-arts" index={2} openInquiry={() => openForVenue("performing-arts")} />,
    <VenueSlide key="expo" slug="expo" index={3} openInquiry={() => openForVenue("expo")} />,
    <VenueSlide key="private-events" slug="private-events" index={4} openInquiry={() => openForVenue("private-events")} />,
  ];
